...
<div class="container">
    ...
    $sql = "insert into blood_information(blood_group,issue_date,owner_id_number,first_name,middle_name,last_name,
                              age,telephone_number,email,next_of_kin,nok_phone_number,nok_relationship)
                              values ('$bloodGroup','$issueDate','$idPass','$firstName','$middleName',
                                      '$lastName','$age','$telephoneNumber','$email','$nextOfKin','$nokTel','$nokRShip')";
    if ($conn->query($sql) == true){
        print "<p>Blood Information added successfully</p>";
        print "<a href='bloodInformation.php' class='btn btn-outline-primary'>Add New Blood Information</a>";
    }
    else{
        print "<p>Organ information not added</p>";
        print "<a href='bloodInformation.php' class='btn btn-danger'>Try Again.</a>";
    }
    mysqli_close($conn);
    ?>
</div>
